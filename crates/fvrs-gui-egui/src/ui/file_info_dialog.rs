        ...
        egui::Grid::new("general_info_grid")
            ...
            .show(ui, |ui| {
                ...
                ui.label(
                    info.full_path
                        .parent()
                        .map_or("不明".to_string(), |p| p.to_string_lossy().to_string()),
                );
                ui.end_row();
                ...
                if info.is_readonly {
                    attributes.push("読み取り専用");
                }
                if info.is_hidden {
                    attributes.push("隠しファイル");
                }
                if info.is_system {
                    attributes.push("システム");
                }
    ...
    fn show_security_info(ui: &mut egui::Ui, info: &DetailedFileInfo) {
        ...
        egui::Grid::new("security_info_grid")
            ...
            .show(ui, |ui| {
                ...
                if info.is_readonly {
                    security_attrs.push("読み取り専用");
                }
                if info.is_hidden {
                    security_attrs.push("隠しファイル");
                }
                if info.is_system {
                    security_attrs.push("システムファイル");
                }
                if security_attrs.is_empty() {
                    security_attrs.push("標準");
                }
