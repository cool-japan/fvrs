...
- ~8.5k lines of Rust (7.2k code SLoC per tokei, post-oxiarc-0.3.4 slim-down) across 5 crates: `fvrs-core`, `fvrs-cli`, `fvrs-gui-egui` (live), `fvrs-plugin-api`, `fvrs-plugins` (both outside the workspace, do not build).
...
**oxiarc 0.3.4 slim-down (2026-07-06):** oxiarc 0.3.4 (published 2026-07-06) is now the **direct backend** — the whole in-repo compat layer written against 0.3.3's gaps was deleted: 7 modules, 3,932 lines (`archive/sevenz.rs` 1,132, `archive/bzip2_compat.rs` 877, `archive/lzh.rs` 664, `archive/lzma_compat.rs` 543, `archive/lzhuf1.rs` 436, `archive/zip_names.rs` 157, `archive/name_codec.rs` 123). Exact delta: +67/−4,079 (net −4,012); `archive.rs` 998 → 918 lines, now a single file with no `archive/` subtree. `archive.rs` calls upstream directly: `SevenZReader` (7z list/extract, CRC-verified, anti/empty/dir handling preserved), `LzhReader`/`LzhWriter`, plain `ZipReader` entry names (upstream does the EFS→UTF-8→Shift_JIS→CP437 chain), `oxiarc_archive::bzip2` for tar.bz2 both directions, and upstream automatic PAX long-name handling via `add_entry_from_header` (fvrs's manual `pax_path_record`/`tar_fallback_name`/`add_tar_entry_safe` are gone). Path sanitization, RAR feature gating, Japanese error prefixes, and the `ArchiveHandler` public API unchanged. Verified: `cargo check --workspace --all-targets` and `cargo clippy --workspace --all-targets --all-features` zero warnings, `--features rar` compiles, `cargo tree` shows all 10 oxiarc crates at 0.3.4. Tests 37 → 24 (14 compat unit tests dropped with their modules; 1 new lh5-compression test added), 24/24 green. Known upstream regression (confirmed by adversarial review, medium): LZH `-lz4-`/`-pm0-` stored entries (LArc/PMarc legacy) list but are silently skipped at extraction (oxiarc 0.3.4 maps them to `UnsupportedMethod`); the deleted compat extracted them byte-exact — track upstream.

### fvrs-gui-egui dependency replacements

- [x] Remove `zip = "2.1"` (`crates/fvrs-gui-egui/Cargo.toml`) → `oxiarc_archive::zip::ZipReader::new(file)?` — list via `.entries() -> &[Entry]` (replaces the `by_index` loop at archive.rs:97), extract via `.extract(&entry)` writing to `entry.sanitized_name()` (**fixes the zip-slip traversal at archive.rs:459**); create via `ZipWriter::new(file)` (`Write`-only, no `Seek` bound) with `add_file`/`add_directory`/`finish` replacing `start_file` + `io::copy` in `add_to_zip` (archive.rs:705-725 — this rewrite also removes 4 `unwrap()` calls there). (0.3.4 note: the interim in-repo Shift_JIS/CP437 name recovery — `zip_names.rs`/`name_codec.rs` — was deleted; plain `ZipReader` entry names are used, upstream now does the EFS→UTF-8→Shift_JIS→CP437 chain, and the ZIP EFS Japanese-name strict roundtrip test stays green.)
...
- [x] Remove `bzip2 = "0.4"` (`crates/fvrs-gui-egui/Cargo.toml`; links C libbz2) → `.tar.bz2` list/extract via `oxiarc_archive::bzip2::decompress_reader(file)?` → `Cursor` → `TarReader`; create via `TarWriter::new(Vec::new())` → `into_inner()` → `bzip2::compress_with_level(&tar_bytes, 9)` → `fs::write` (oxiarc's bzip2 encoder is block-API, buffer route is the clean mapping). (0.3.4 note: the interim spec-correct in-repo codec `bzip2_compat.rs` — written because oxiarc-bzip2 0.3.3 was incompatible with real bzip2 — was deleted; `oxiarc_archive::bzip2` 0.3.4 is used directly in both directions and was validated against real libbz2: fvrs-created tar.bz2 passes `bzip2 -t` and bsdtar-extracts byte-exact, real-bzip2 archives list/extract byte-exact.)
- [x] Remove `sevenz-rust = "0.6"` (deprecated upstream; `crates/fvrs-gui-egui/Cargo.toml`) → `oxiarc_archive::sevenz::SevenZReader::new(file)?` — `.entries()` gives a **real 7z listing**; delete the fake `list_7z_contents` that extracts the whole archive into `std::env::temp_dir()` as a "listing" side effect (archive.rs:329-346); extract via `.extract(index) -> Vec<u8>`. (0.3.4 note: the interim in-repo reader — `sevenz.rs` + `lzma_compat.rs` — was deleted; upstream `SevenZReader` is used directly with full coder parity (Copy/LZMA/LZMA2/Deflate/BZip2/Delta/BCJ-x86, BCJ2 rejected in both), and the 7z extraction byte-compare test is enabled, including an LZMA-folder fixture with embedded real liblzma bytes — `sevenz_lzma_folder_extracts_correct_content`, folder-CRC verified.)
...
- [x] Remove `delharc = "0.6"` (`crates/fvrs-gui-egui/Cargo.toml`) → `oxiarc_archive::lzh::LzhReader::new(file)?` — `.entries()` replaces the `next_file()` loop (archive.rs:139); `.extract(&entry, &mut writer)` verifies CRC internally (archive.rs:482, also zip-slip-guard the output path). Bonus (optional): `LzhWriter` makes LZH **creation** possible, replacing the polite error stub at archive.rs:682. (0.3.4 note: the interim in-repo parser/writer — `lzh.rs` + `lzhuf1.rs` — was deleted; upstream `LzhReader`/`LzhWriter` are used directly. `create_lzh` was **reverted from the `-lh0-` Store workaround to real `-lh5-` compression** via `LzhWriter::add_file_raw` (upstream-identical `-lh0-` fallback when compression doesn't shrink, source mtimes preserved, level-2 Shift_JIS headers) — proven by the new `lzh_compresses_compressible_input` test. The LZH roundtrip is now fully STRICT incl. Japanese names; unsupported methods still list-and-skip via `OxiArcError::UnsupportedMethod`. Regression: `-lz4-`/`-pm0-` stored entries no longer extract, see the 0.3.4 slim-down note above.)
